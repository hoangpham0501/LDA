library(lattice)
# Reorder both axes by major category
order_by_major_index <- 1:49
names(order_by_major_index) <- rownames(theta_diagnostic)
order_by_major_index <- order_by_major_index[category_levels$pretty ]
theta_diagnostic_ordered_by_major <- unique(theta_diagnostic[order_by_major_index, order_by_major_index])

# Transpose the matrix and reverse rows for orientation like in GS04
theta_diagnostic_plot <- t(theta_diagnostic_ordered_by_major[nrow(theta_diagnostic_ordered_by_major):1 ,])
...
print(levelplot(theta_diagnostic_plot, xlab = NULL, ylab = NULL , 
                scales = list( tck = 1, x = list(rot = 45),
                               y = list(col = colors_plot)),
                cuts = 12,
                col.regions = gray(20:0/20),
                colorkey = list (space = "right", tick.number = 10) ,
