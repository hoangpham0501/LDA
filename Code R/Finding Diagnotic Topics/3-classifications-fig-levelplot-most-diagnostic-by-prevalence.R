library(lattice)
categories_prevalence <- table(categories_2016$pretty)
# make a copy our main theta matrix with added counts in rownames
theta_diagnostic_with_prevalence <- theta_diagnostic
rownames(theta_diagnostic_with_prevalence) <- paste(rownames(theta_diagnostic),
                                                    categories_prevalence, sep=":")
# prepare index to reorder theta matrix
categories_ordered_by_prevalence <- names(sort(categories_prevalence, decreasing = TRUE))
theta_index <- 1:49
names(theta_index) <- rownames(theta_diagnostic)
theta_index <- theta_index[categories_ordered_by_prevalence]

# reorder theta matrix
theta_diagnostic_ordered_by_prevalence <- theta_diagnostic_with_prevalence[rev(theta_index),theta_index ]
