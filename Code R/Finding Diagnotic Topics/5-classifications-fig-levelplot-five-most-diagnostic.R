library(lattice)
# Reordered topics by ratio
topics_by_ratio_reordered <- topics_by_ratio[1:5 , order_by_major_index ]

topics_most_diagnostic_five <- unique(as.vector(topics_by_ratio_reordered ))

# Convert the data (bottom to top)
theta_plot <- t(theta_mean_ratios[rev(order_by_major_index),topics_most_diagnostic_five])
