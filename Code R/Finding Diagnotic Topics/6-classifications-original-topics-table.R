...
terms_topics_unique <- matrix (data= c( c("insect","myb","pheromone","lens","larvae"),
                               ...
                               dimnames = list(1:5 , paste("Topic",c(217 ,274 ,126 ,63 ,200 ,209 ,
                                                                     42 ,2 ,280 ,15 ,64 ,102 ,
...
for(i in 1:( topics_total %/% topics_per_tab)) {
  xtable_raw <- xtable(terms_topics_unique[,( topics_per_tab*(i -1) + 1):(topics_per_tab * i), 
                                           drop = FALSE ])
