...
## Create Hadoop temp directories for Namenode at Master machine
...
<configuration>
	...
	<property>
		...
		<value>hdfs:///usr/local/hadoop_tmp/hdfs/datanode</value>
	</property>
</configuration>
...
#Copying/Sharing Hadoop config files to rest all nodes using rsync
Ex:
...
Adding the name of slave machine in this file
Ex:
Slave1
Slave2
......

# Copying ssh key for setting up password ssh access from Master to Slave node
hduser@master:~$ ssh-copy-i $HOME/.ssh/id_rsa.pub hduser@slave1
hduser@master:~$ ssh-copy-i $HOME/.ssh/id_rsa.pub hduser@slave2
....
...
#Starting up MapReduce deamons
hduser@master:~$ start-yarn.sh

**** Testing Hadoop is running
$ jps